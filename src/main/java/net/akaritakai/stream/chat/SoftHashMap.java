...
public class SoftHashMap<K,V>
    ...
    private SoftHashMap.Entry<K,V>[] newTable(int n) {
        return (SoftHashMap.Entry<K,V>[]) new SoftHashMap.Entry<?,?>[n];
    }
     ...
     *
     * @apiNote
     * To create a {@code SoftHashMap} with an initial capacity that accommodates
     * an expected number of mappings, use {@link #newSoftHashMap(int) newSoftHashMap}.
     *
     ...
     */
    private boolean matchesKey(SoftHashMap.Entry<K,V> e, Object key) {
    ...
    private void expungeStaleEntries() {
        for (Object x; (x = queue.poll()) != null; ) {
            synchronized (queue) {
                @SuppressWarnings("unchecked")
                SoftHashMap.Entry<K,V> e = (SoftHashMap.Entry<K,V>) x;
                int i = indexFor(e.hash, table.length);

                SoftHashMap.Entry<K,V> prev = table[i];
                SoftHashMap.Entry<K,V> p = prev;
                while (p != null) {
                    SoftHashMap.Entry<K,V> next = p.next;
                    if (p == e) {
                        if (prev == e)
                            table[i] = next;
                        else
     ...
     */
    private SoftHashMap.Entry<K,V>[] getTable() {
        expungeStaleEntries();
        return table;
    }
    ...
    public V get(Object key) {
        ...
        int h = hash(k);
        SoftHashMap.Entry<K,V>[] tab = getTable();
        int index = indexFor(h, tab.length);
        SoftHashMap.Entry<K,V> e = tab[index];
        while (e != null) {
     ...
     */
    SoftHashMap.Entry<K,V> getEntry(Object key) {
        Object k = maskNull(key);
        int h = hash(k);
        SoftHashMap.Entry<K,V>[] tab = getTable();
        int index = indexFor(h, tab.length);
    ...
    public V put(K key, V value) {
        ...
        for (SoftHashMap.Entry<K,V> e = tab[i]; e != null; e = e.next) {
        ...
        modCount++;
        SoftHashMap.Entry<K,V> e = tab[i];
        tab[i] = new SoftHashMap.Entry<>(k, value, queue, h, e);
        if (++size > threshold)
            resize(tab.length * 2);
        return null;
    }
    ...
    void resize(int newCapacity) {
        SoftHashMap.Entry<K,V>[] oldTable = getTable();
        ...
        SoftHashMap.Entry<K,V>[] newTable = newTable(newCapacity);
    ...
    public V remove(Object key) {
        ...
        SoftHashMap.Entry<K,V>[] tab = getTable();
        int i = indexFor(h, tab.length);
        SoftHashMap.Entry<K,V> prev = tab[i];
        SoftHashMap.Entry<K,V> e = prev;

        while (e != null) {
            SoftHashMap.Entry<K,V> next = e.next;
            if (h == e.hash && matchesKey(e, k)) {
                modCount++;
                size--;
                if (prev == e)
    ...
    private static class Entry<K,V> extends SoftReference<Object> implements Map.Entry<K,V> {
        ...
        Entry(Object key, V value,
              ...
              int hash, SoftHashMap.Entry<K,V> next) {
            super(key, queue);
            this.value = value;
            this.hash  = hash;
            this.next  = next;
    ...
    private abstract class HashIterator<T> implements Iterator<T> {
        private int index;
        private SoftHashMap.Entry<K,V> entry;
        private SoftHashMap.Entry<K,V> lastReturned;
    ...
    private class KeySet extends AbstractSet<K> {
        public Iterator<K> iterator() {
            return new SoftHashMap.KeyIterator();
        }
        ...
        public Spliterator<K> spliterator() {
            return new SoftHashMap.KeySpliterator<>(SoftHashMap.this, 0, -1, 0, 0);
        }
    }
    ...
    private class Values extends AbstractCollection<V> {
        public Iterator<V> iterator() {
            return new SoftHashMap.ValueIterator();
        }
        ...
        public Spliterator<V> spliterator() {
            return new SoftHashMap.ValueSpliterator<>(SoftHashMap.this, 0, -1, 0, 0);
        }
    }
    ...
    public Set<Map.Entry<K,V>> entrySet() {
        ...
        return es != null ? es : (entrySet = new SoftHashMap.EntrySet());
    }

    private class EntrySet extends AbstractSet<Map.Entry<K,V>> {
        public Iterator<Map.Entry<K,V>> iterator() {
            return new SoftHashMap.EntryIterator();
        }
        ...
        public Spliterator<Map.Entry<K,V>> spliterator() {
            return new SoftHashMap.EntrySpliterator<>(SoftHashMap.this, 0, -1, 0, 0);
        }
    }
    ...
    public void forEach(BiConsumer<? super K, ? super V> action) {
        ...
        SoftHashMap.Entry<K, V>[] tab = getTable();
        for (SoftHashMap.Entry<K, V> entry : tab) {
            while (entry != null) {
    ...
    static final class KeySpliterator<K,V>
            extends SoftHashMap.SoftHashMapSpliterator<K,V>
        ...
        public SoftHashMap.KeySpliterator<K,V> trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid) ? null :
                    new SoftHashMap.KeySpliterator<>(map, lo, index = mid, est >>>= 1,
                            expectedModCount);
        }
    ...
    static final class ValueSpliterator<K,V>
        ...
        public SoftHashMap.ValueSpliterator<K,V> trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid) ? null :
                    new SoftHashMap.ValueSpliterator<>(map, lo, index = mid, est >>>= 1,
                            expectedModCount);
        }
    ...
    static final class EntrySpliterator<K,V>
        ...
        public SoftHashMap.EntrySpliterator<K,V> trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid) ? null :
                    new SoftHashMap.EntrySpliterator<>(map, lo, index = mid, est >>>= 1,
                            expectedModCount);
        }
    ...
    /**
     * Creates a new, empty SoftHashMap suitable for the expected number of mappings.
     ...
     * @since 19
     */
    public static <K, V> SoftHashMap<K, V> newSoftHashMap(int numMappings) {
        if (numMappings < 0) {
            throw new IllegalArgumentException("Negative number of mappings: " + numMappings);
        }
        return new SoftHashMap<>(calculateHashMapCapacity(numMappings));
    }
